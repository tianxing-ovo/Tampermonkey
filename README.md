...
- **数据与逻辑分离重构**：将上下文翻译规则（`contextTranslations`）从 JS 脚本中移除并转移到外部资源 `translations.json` 中，实现规则的热更新。
- **歧义规则精简**：精简了 `save` 和 `type` 词条的规则库，将默认翻译收敛进字典，大幅减少无意义的选择器匹配。
- **IDE 友好优化**：为动态加载的规则数组编写了 JSDoc 类型批注，解决 WebStorm / VSCode 的 `unresolved variable` 警告并重获类型提示。
- **向下兼容处理**：代码层面对旧版本扁平字典结构进行了兜底，防止因缓存未同步引发的脚本崩溃。
